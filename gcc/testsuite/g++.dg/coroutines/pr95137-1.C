...
template <typename... T>
struct promise_base_with_type : protected promise_base {
...
struct future : public future_base {
    ...
    Result then_impl_nrvo(Func&& func) noexcept {
        ...
        [&]() noexcept {
            ...
            schedule(
                fut.get_promise(), [func = std::forward<Func>(func)](pr_type& pr, future_state<T...>&& state) mutable {
                    if (state.failed()) {
                        assert(0);
                    } else {
...
int main(int argc, char** argv) {
    ...
    make_ready_future<>().then([func = std::move(func)]() mutable {
        return func().then([] {
            _exit(0);
        });
    });
