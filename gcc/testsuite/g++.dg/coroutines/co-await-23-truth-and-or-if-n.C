...
struct IntAwaiter {
  ...
  int await_resume () {
   PRINTF ("IntAwaiter await_resume %d\n", v); return v; }
  ~IntAwaiter () { PRINT ("~IntAwaiter"); }
};
...
struct BoolAwaiter {
  ...
  BoolAwaiter (bool _v) : v(_v) {
   PRINTF ("BoolAwaiter : %d\n", v); }
  bool await_ready () { return false; }
  void await_suspend (coro::coroutine_handle<>) {}
  bool await_resume () { v = !v; return v; }
  ~BoolAwaiter () { PRINT ("~BoolAwaiter"); }
};
...
my_coro (int t)
{
  bool x = co_await IntAwaiter (t) == 5
  	   || co_await BoolAwaiter (false)
  	   || co_await IntAwaiter (t) == 10;
